...
PLX_DEVICE_OBJECT PCIDevHandle;//Device;
static udword IOVal ;
...
{
	//if(PlxPci_IoPortWrite(&PCIDevHandle, p, BitSize32, &d) != ApiSuccess)
	if(PlxPci_IoPortWrite(&PCIDevHandle, p, &d,4,BitSize32) != ApiSuccess)
	{
...
{
	//if(PlxPci_IoPortWrite(&PCIDevHandle, p, BitSize16, &d) != ApiSuccess)
	if(PlxPci_IoPortWrite(&PCIDevHandle, p, &d,4,BitSize16) != ApiSuccess)
	{
...

{
	...
	//if(PlxPci_IoPortRead(&PCIDevHandle, p, BitSize32, &d) != ApiSuccess)
	if(PlxPci_IoPortRead(&PCIDevHandle, p, &d,4,BitSize32) != ApiSuccess)
	{
...

{
	...
	//if(PlxPci_IoPortRead(&PCIDevHandle, p, BitSize16, &d) != ApiSuccess)
	if(PlxPci_IoPortRead(&PCIDevHandle, p, &d,4,BitSize16) != ApiSuccess)
	{
...

{
	...
	RETURN_CODE       rcode;//rc;
    PLX_DEVICE_KEY    devicelocinfo;//Key;


	/* Find the card.
	*/
	(devicelocinfo . bus) = -1 ;
	(devicelocinfo . slot) = -1 ;
	...
	if(PlxPci_DeviceFind(&devicelocinfo, (U8)devicenum) != ApiSuccess)
	{
	...
	*/
	if(DoIO32)
	{
		p = PlxPci_PciRegisterRead((devicelocinfo . bus), (devicelocinfo . slot),(devicelocinfo . function), R_5I20PCICFGIO32, &rcode) ;
		if(rcode != ApiSuccess)
		{
	...
	{
		p = PlxPci_PciRegisterRead((devicelocinfo . bus), (devicelocinfo . slot),(devicelocinfo . function), R_5I20PCICFGIO16, &rcode) ;
		if(rcode != ApiSuccess)
		{
