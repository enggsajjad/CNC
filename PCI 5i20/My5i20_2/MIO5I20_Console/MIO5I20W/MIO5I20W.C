...

{
	if(Mapped)
	{
		PlxPci_PciBarUnmap(&PCIDevHandle, A_PCIMemBase) ;
	}
	if(Opened)
	{
...

{
	...
	*/
	if(DoIO32)
	{
		rcode = PlxPci_PciBarMap(&PCIDevHandle, BRNUM_5I20MEM32, &A_PCIMemBase) ;
		if(rcode != ApiSuccess)
		{
	...
	{
		rcode = PlxPci_PciBarMap(&PCIDevHandle, BRNUM_5I20MEM16, &A_PCIMemBase) ;
		if(rcode != ApiSuccess)
		{
