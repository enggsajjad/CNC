...
{
	if(PlxPci_IoPortWrite(&PCIDevHandle, (P_FPGAIOBase + p), &d,4,BitSize32) != ApiSuccess)
	{
...

{
	...
	if(PlxPci_IoPortRead(&PCIDevHandle, (P_FPGAIOBase + p), &d,4, BitSize32) != ApiSuccess)
	{
...

{
	...
	/* Find the card.*/
	(devicelocinfo . bus) = 4;//-1 ;
	(devicelocinfo . slot) = 2;//-1 ;
